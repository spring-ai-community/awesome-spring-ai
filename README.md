...
- [JamJet Java Runtime](https://github.com/jamjet-labs/jamjet-runtime-java) - A pure-Java durability and governance runtime that attaches to Spring AI as a `ChatClient` advisor (and to LangChain4j). Adds crash recovery, event-sourced execution, replay-based testing, and audit trails to any Spring AI app with zero code changes and no sidecar. The runtime runs inside your JVM and shares your DataSource. Part of the cross-language [JamJet](https://github.com/jamjet-labs/jamjet) safety layer for agents: policy, approval gates, budget caps, and audit across Rust, Python, and the JVM. (Apache-2.0, Maven `dev.jamjet`, Java 21+, Spring Boot 3 / Spring AI 1.0+)
